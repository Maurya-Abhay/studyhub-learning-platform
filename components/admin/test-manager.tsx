...
function TestList({ items, query, setQuery, edit, remove }: { items: Test[]; query: string; setQuery: (value: string) => void; edit: (item: Test) => void; remove: (id: string) => void }) { return <section className="surface card"><div className="search"><Search size={15} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search tests..." /></div>{items.length ? items.map((item) => <div className="list-row" key={item.id}><div><strong>{item.title}</strong><div className="small muted">{item.duration_minutes} min · {item.passing_score}% pass · unlock {item.unlock_days}d</div></div><div className="topic-actions"><span className={`chip ${item.published ? 'active' : ''}`}>{item.published ? 'Published' : 'Draft'}</span><button className="icon-btn" onClick={() => edit(item)} aria-label="Edit test"><Pencil size={14} /></button><button className="icon-btn" onClick={() => remove(item.id)} aria-label="Delete test"><Trash2 size={14} /></button></div></div>) : <div className="empty">No tests found.</div>}</section>; }
