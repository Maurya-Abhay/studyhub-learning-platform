  ...
  if (!valid || !certificate) {
    return (
      <main className="certificate-page">
        <Link href={user ? '/dashboard/certificates' : '/'} className="btn secondary small certificate-back">
          <ArrowLeft size={14} /> Back to certificates
        </Link>
        <section className="certificate-invalid surface">
          <Award size={40} />
          <h1>Certificate not found</h1>
          <p>No active certificate matches this verification code.</p>
        </section>
      </main>
    );
  }
  ...
  return (
    <main className="certificate-page">
      <div className="certificate-actions">
        <Link href={user ? '/dashboard/certificates' : '/'} className="btn secondary small">
          <ArrowLeft size={14} /> Back to certificates
        </Link>
        <CertificateActions certificateCode={certificate.certificate_code} />
      </div>
      <article className="certificate-sheet">
        <div className="certificate-ribbon certificate-ribbon-top" />
        <div className="certificate-ribbon certificate-ribbon-bottom" />
        <div className="certificate-inner">
          <div className="certificate-topline">
            <div className="certificate-brand">
              <span className="certificate-brand-mark">S</span>
              <span><strong>Study</strong>Hub</span>
            </div>
          ...
          <div className="certificate-qr">
            <span /><span /><span /><span />
            <b>{certificate.certificate_code.slice(-6)}</b>
          </div>
          ...
          <div className="certificate-details">
            <div><span>Enrollment date</span><strong>{date}</strong></div>
            <div><span>Completion date</span><strong>{date}</strong></div>
            <div><span>Certificate ID</span><strong>{certificate.certificate_code}</strong></div>
            <div><span>Final score</span><strong>{certificate.score}%</strong></div>
          </div>
          <div className="certificate-footer">
            <div>
              <div className="certificate-signature">StudyHub</div>
              <span>Learning platform</span>
            </div>
            <div className="certificate-valid">
              <CheckCircle2 size={15} /> 
              <span>Verified credential<br /><small>studyhub certificate registry</small></span>
            </div>
            <div className="certificate-signature">StudyHub</div>
          </div>
        </div>
      ...
      <p className="certificate-verification">
        <ShieldCheck size={14} /> Verified certificate · ID {certificate.certificate_code}
      </p>
    </main>
  );
}