...
export default async function Page({ searchParams }: { searchParams: Promise<{ topic?: string }> }) {
  ...
  const [{ data: noteRows }, { data: enrollments }] = await Promise.all([
  ...
  return <DashboardShell><div className="eyebrow">My learning</div><h1 className="title" style={{ fontSize: 38 }}>{selectedTopic ? selectedTopic.title : 'Notes & roadmap'}</h1><p className="subtitle">{selectedTopic ? 'Study this topic inside your learner portal.' : 'Keep your saved notes and course roadmap together.'}</p>{selectedTopic && selectedLibrary ? <div className="dashboard-topic-reader"><TopicWorkspace topic={selectedTopic} category={selectedCategory} prev={selectedLibrary.topics[selectedLibrary.topics.findIndex((item) => item.id === selectedTopic.id) - 1]} next={selectedLibrary.topics[selectedLibrary.topics.findIndex((item) => item.id === selectedTopic.id) + 1]} initialProgress={selectedProgress} initialNote={selectedNote} /></div> : <><div className="dashboard-section-heading"><h2>Roadmap</h2><Link className="small link" href="/dashboard/roadmap">View full roadmap <span aria-hidden="true">→</span></Link></div><RoadmapView courses={roadmaps} /><section className="surface card notes-reader-surface"><div className="dashboard-section-heading"><h2>Saved notes</h2><span className="small muted">{notes.length} {notes.length === 1 ? 'note' : 'notes'}</span></div><NotesPanel notes={notes} /></section></>}</DashboardShell>;
}
