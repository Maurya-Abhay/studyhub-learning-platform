...
export default async function PrivateCertificatePage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  const supabase = await createClient();
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return null;
  const { data: row } = await supabase.from('certificates').select('certificate_code,score,issued_at,revoked_at,user_id,course_id').eq('certificate_code', id).eq('user_id', user.id).maybeSingle();
  ...
  const date = certificate ? new Date(certificate.issued_at).toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' }) : null;

  return <DashboardShell><main className="certificate-page dashboard-certificate-page">
    <div className="certificate-actions"><Link href="/dashboard/certificates" className="btn secondary small"><ArrowLeft size={14} /> Back to certificates</Link>{valid && certificate ? <CertificateActions certificateCode={certificate.certificate_code} /> : null}</div>
    {!valid || !certificate ? <section className="certificate-invalid surface"><Award size={40} /><h1>Certificate not found</h1><p>This certificate is not available in your account.</p></section> : <>
      <article className="certificate-sheet">
        ...
        <div className="certificate-inner">
          ...
          <div className="certificate-footer"><div><div className="certificate-signature">StudyHub</div><span>Learning platform</span></div><div className="certificate-valid"><CheckCircle2 size={15} /> <span>Verified credential<br /><small>private learner credential</small></span></div><div className="certificate-signature">StudyHub</div></div>
        </div>
      </article>
      <p className="certificate-verification"><ShieldCheck size={14} /> Private certificate · ID {certificate.certificate_code}</p>
    </>}
  </main></DashboardShell>;
}
