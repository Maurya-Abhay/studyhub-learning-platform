...
import { requireAdmin, requireLearner } from '@/lib/guards';
...
export async function ProfilePage({ adminOnly = false }: { adminOnly?: boolean } = {}) {
  const user = adminOnly ? await requireAdmin() : await requireLearner();
  ...
  const qrDataUrl = await QRCode.toDataURL(`studyhub://profile/${user.id}`, { width: 120, margin: 1, errorCorrectionLevel: 'M' });

  return <DashboardShell admin={role === 'admin'}>
    <div className={`profile-page-content ${role === 'admin' ? 'admin-profile' : 'student-profile'}`}>
      <div className="section-head profile-page-head"><div /><Link className="btn secondary small" href={role === 'admin' ? '/admin' : '/dashboard'}><CircleUserRound size={14} /> Dashboard</Link></div>
      <section className="profile-hero surface">
        <div className="profile-identity"><div className="profile-avatar" aria-label={`${displayName} avatar`}>{initials || 'L'}</div><div className="profile-identity-copy"><div className="profile-name-line"><h2>{displayName}</h2><span className="profile-verified"><CheckCircle2 size={13} /> {role === 'admin' ? 'Active administrator' : 'Active learner'}</span></div><p><Mail size={14} /> {user.email}</p><div className="profile-meta"><span><CalendarDays size={13} /> Joined {formatDate(profile?.created_at)}</span><span><ShieldCheck size={13} /> {role === 'admin' ? 'Administrator' : 'Student account'}</span></div></div></div>
        <div className="profile-level"><div className="profile-level-top"><span>{role === 'admin' ? 'Admin access' : 'Current level'}</span><strong>{role === 'admin' ? 'Full control' : `Level ${level}`}</strong></div><div className="profile-level-track"><span style={{ width: role === 'admin' ? '100%' : `${levelProgress}%` }} /></div><small>{role === 'admin' ? 'Content, users, assessments and settings' : `${5 - Math.floor(learningPoints % 5)} points to the next level`}</small></div>
        <div className="profile-scan-card"><div className="profile-scan-visual"><img src={qrDataUrl} alt="Profile QR code" /></div><div><strong>StudyHub ID</strong><small>Scan to identify this {role === 'admin' ? 'administrator' : 'learner'}</small><code>{profileId}</code></div></div>
      </section>
      {role === 'admin' ? <section className="profile-metrics admin-profile-metrics" aria-label="Administrator statistics"><div className="profile-metric"><span className="profile-metric-icon blue"><ShieldCheck size={17} /></span><div><strong>ADMIN</strong><small>Account role</small></div></div><div className="profile-metric"><span className="profile-metric-icon violet"><Layers3 size={17} /></span><div><strong>FULL</strong><small>Platform access</small></div></div><div className="profile-metric"><span className="profile-metric-icon green"><CheckCircle2 size={17} /></span><div><strong>ACTIVE</strong><small>Account status</small></div></div></section> : <section className="profile-metrics" aria-label="Learning statistics">
